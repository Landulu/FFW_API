    ...
    public function processQuery($urlArray, $method){
        ...
        */
        ...
        if ( count($urlArray) == 1 && $method == 'POST') {
            ...
            $newComments = services\CommentsService::getInstance()->create(new Comment($obj));
            if($newComments) {
        ...
        */
        if ( count($urlArray) == 2 && ctype_digit($urlArray[1]) && $method == 'PUT') {
            ...
            $newComments = services\CommentsService::getInstance()->update(new Comment($obj),$urlArray[1]);
            if($newComments) {
                http_response_code(201);
                return $newComments;
            } else {
